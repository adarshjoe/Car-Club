...
def payForOrderAction(request):
    uid=user_tb.objects.get(id=request.session['user_id'])
    order = order_tb.objects.filter(id=request.GET['order_id'])
    oid = order_tb.objects.get(id=request.GET['order_id'])
    sid = order[0].shop_id
    request.session['page'] = request.GET['orderpage']
    key = random.randint(100000, 999999)
    payment = payment_tb(order_id=oid, amount=request.GET['amount'], transaction_key=key, date=datetime.date.today(),
                         user_id=uid, shop_id=sid)
    payment.save()
